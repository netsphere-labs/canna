...
cannawc*
WStrncpy(cannawc* ws1, const cannawc* ws2, size_t cnt)
{
  wchar_t *ws;
...
size_t
CANNA_mbstowcs(cannawc* dest, const char* src, size_t destlen)
{
    assert(src);
        ...
        if ( !(ec & 0x80) )
            ...
            case 0x8f: /* SS3 */
                if ( !(s[0] & 0x80) || !(s[1] & 0x80))
                    return -1;
                if (dest) dest[j] = (s[0] << 8) | (s[1] & 0x7f);
                s += 2;
                break;
            default: // Kanji
                if ( !(*s & 0x80) )
                    return -1;
                if (dest) dest[j] = (ec << 8) | *s;
                s++;
                break;
            }
        }
...
// 引数の順序が違う.
size_t
CNvW2E(const cannawc* src, int srclen, char* dest, size_t destlen)
{
    assert(src);
...
size_t
CANNA_wcstombs(char* dest, const cannawc* src, size_t destlen)
{
    assert(src);
