 ...
 *
 * 引き数	なし
 * @return  0:まあ正常、 -1:とことん不良
 */
int KanjiInit()
{
    // サーバのホスト名
    const char* serv_ptr;
    const char* kodmesg = "" /* 辞書の種別毎のメッセージ */;
    int con;
    struct dicname *stp;
    ...
    defaultContext = RkwInitialize(serv_ptr); // サーバに接続
 ...
 *
 * @param d      uiContext
 * @param len    len が指定されていたら文節長をその長さにする。
 * @param kanji  kanji が指定されていたら単文節変換して、
...
/**
 * tanContext を yomiContext にして変換の準備をする
 */
